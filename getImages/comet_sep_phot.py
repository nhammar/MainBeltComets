...
def iterate_thru_images(family_name, object_name, expnum_p, username, password, ap, th, p_ra, p_dec):
    ...
    for item in os.listdir(_STAMPS_DIR):
        if item.endswith('.fits'):
            if (item.split('_')[0] == object_name) and (item.split('_')[1] == expnum_p):
                with fits.open('{}/{}'.format(_STAMPS_DIR, item)) as hdulist:
                    ...
                    objs_met_cond, r_err, p_f, p_mag = iden_good_neighbours(family_name, object_name, transients,
                                                                            header, r_sig, p_ra, p_dec, expnum_p,
                                                                            mag_list_jpl, ra_dot, dec_dot)
...
def append_table(objs, header, p_ra, p_dec):
    ...
    table['mag'] = -2.5 * np.log10(table['flux'].values) + zeropt
...
def check_involvement(object_data, catalogue, r_err, header):
    ...
    pvwcs = wcs.WCS(header)
    # TEMPORARY: for comet search
    size_x = 2000  # header['NAXIS1']
    size_y = 4000  # header['NAXIS2']
