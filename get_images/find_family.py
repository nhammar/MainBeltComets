...
_OUTPUT_DIR = config._FAMILY_LISTS
if not os.path.isdir(_OUTPUT_DIR):
    os.makedirs(_OUTPUT_DIR)

_BASEURL_NUMB_FAM = 'http://hamilton.dm.unipi.it/~astdys2/propsynth/all.famtab'
_BASE_URL_NUMB_MEM = 'http://hamilton.dm.unipi.it/~astdys2/propsynth/all.members'
_BASE_URL_FAMREC = 'http://hamilton.dm.unipi.it/~astdys2/propsynth/all.famrec'
...
def main():
    ...
    parser.add_argument("--status",
                        action='store',
                        default=4,
                        help='Family status, options: 0 not in family, 1 inner MB, 2 MB, 3 outer MB')
    parser.add_argument('--fromfile',
                        action='store',
                        default=True,
                        help='Whether to find family members from search or from input file')
...
def parse_for_all(mba_list, status, fromfile=True):
    ...
    print "----- Searching for objects in AstDys -----"
